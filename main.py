...
def resolve_universe(client: AlpacaClient) -> tuple[list[str], list[str]]:
    ...
    tiene disponibles ahora mismo. Si falla, el bot sigue solo con acciones
    en vez de no arrancar."""
    stocks = config.symbols_stocks
    if config.symbols_crypto != ["ALL"]:
        return stocks, config.symbols_crypto

    signal.alarm(30)
    try:
        crypto = client.get_tradable_crypto_symbols()
    except Exception as e:
        print(f"No se pudo resolver el universo de cripto ({type(e).__name__}: {e}), sigo solo con acciones", flush=True)
        crypto = []
    finally:
        signal.alarm(0)
...
def run_cycle(client: AlpacaClient, risk: RiskManager, params: StrategyParams, stocks: list[str], crypto: list[str]):
    ...
    if stocks and market_open:
        ...
        stock_bars = client.get_bars_batch(stocks, crypto=False, lookback_days=20)
    ...
    if crypto:
        ...
        crypto_bars = client.get_bars_batch(crypto, crypto=True, lookback_days=20)
...
def main():
    print("=== VERSIÓN DE CÓDIGO: 2026-09-08-v5-lotes-pequenos ===", flush=True)
