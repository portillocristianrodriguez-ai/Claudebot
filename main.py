...
def _timeout_handler(signum, frame):
    raise SymbolTimeout("tardó más de 20s, probablemente colgado en una llamada de red")
    ...
    for symbol in all_symbols:
        ...
        try:
            ...
            if not is_crypto and not client.is_market_open():
                print(f"{symbol}: mercado cerrado, se salta", flush=True)
                continue

            print(f"{symbol}: consultando datos de mercado...", flush=True)
            df = client.get_bars(symbol)
            print(f"{symbol}: datos recibidos ({len(df)} velas)", flush=True)
            if df.empty or len(df) < params.sma_slow + 1:
                print(f"{symbol}: datos insuficientes ({len(df)} velas, hacen falta {params.sma_slow + 1})", flush=True)
                continue
            ...
            row = df.iloc[-1]
            print(
                f"{symbol}: precio={last_price:.2f} sma_fast={row['sma_fast']:.2f} "
                f"sma_slow={row['sma_slow']:.2f} rsi={row['rsi']:.1f} "
                f"señal={signal_} posición={'sí' if position else 'no'}",
                flush=True,
            )
            ...
            if signal_ == "buy" and position is None:
                ...
                if qty > 0:
                    ...
                    print(f"{symbol}: señal de compra pero tamaño calculado fue 0 (revisar ATR/riesgo)", flush=True)
            ...
            print(f"{symbol}: ERROR evaluando símbolo: {type(e).__name__}: {e}", flush=True)
            continue
        finally:
            signal.alarm(0)
